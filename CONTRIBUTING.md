...
To run all tests, at the project top level, `pdm run python runtests.py`.
...
All three can be addressed by complementing in-tree testing with some form of [CI](https://en.wikipedia.org/wiki/Continuous_integration). As of `mcpyrate` 3.3.0, a CI workflow was added to do this; it runs automatically on any commit to `master`.

However, as of 3.5.3, test coverage is much less than optimal; this is tracked in issue [#2](https://github.com/Technologicat/mcpyrate/issues/2). While extending tests, we must consider which parts of the library are stable enough to justify drawing a semi-permanent interface line, because introducing tests tends to ossify the design of the system being tested.
